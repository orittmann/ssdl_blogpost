...
After reading this blog post and engaging with the applied exercises, readers should be able to:
 ...
 - calculate a measure of gesticulation and posture
...
1. [**Loading Pose Estimation Output into R**](#third-section)
1. [**Organizing Pose Estimation Output in a Time-Series Data Set**](#fourth-section)
1. [**Visualizing Pose Estimation Data**](#fifth-section)
1. [**Calculating Measure of Gesticulation**](#sixth-section)
1. [**Calculating Measure of Posture**](#seventh-section)
1. [**Wrapping Up**](#eighth-section)



### What Are Pose Estimation Models and Why Are They Helpful For Analyzing Body Language?

How voters perceive a political speech does not only depend on the textual content of a  speech, but also on the speakers' nonverbal behavior. Two speeches with identical text can produce dramatically different effects on listeners based solely on their nonverbal delivery. Thus, to gain a comprehensive picture of political speeches, we should take its nonverbal elements into account. This blog post focuses on one aspect of nonverbal communication: body language.

Politicians' body language during political speeches is encoded in video recordings. This poses a problem if we want to analyse body language on a large scale: As political scientists, we are not used to working with videos as a data source. If we think about videos as data, we are confronted with large amounts of highly unstructured data. Videos are series of image frames, which itself consist of pixels. Individual pixels have no informational value and we cannot use the data analysis tools we are familiar with to analyse them. If we want to computationally analyze video recordings, our challenge is to extract the information that we are interested in from the videos and organize it in a data format that we can actually work with.
...
![Upper panel shows four single video frames, while the lower panel depicts the corresponding body keypoints.](figures/movenet_demo_Brehm.png)

As you can see, pose estimation essentially extracts the information that we are interested in---information on the speaker's body language---from the frames while discarding all other information that is less interesting for this task. 
...
The two videos that we would like to analyze are located at

  + `ssdl_body_language/videos/speech1_gabriela_heinrichs.mp4`
  + `ssdl_body_language/videos/speech2_klaus_ernst.mp4`. 
  
The repository also includes the the pre-trained pose estimation model that we will use to analyze the videos:

  + `ssdl_body_language/model/lite-model_movenet_singlepose_thunder_3.tflite`. 
  
You can find more details on MoveNet [here](https://www.tensorflow.org/hub/tutorials/movenet).
...
Before we implement the model, we need to load all necessary dependencies in Python:
...
The first two columns represent the y- and x-coordinates of the 17 body key points on a scale between 0 and 1:
...
Key point data:
  ...
  - ...
  - `kp17_y` = y-coordinate of key point 17 (right ankle)
...
image <- imager::load.image(paste0("videos/frames_speech2/",
                                   frame_filename))
```
...
kp_x <- t(keypoints[, stringr::str_detect(names(keypoints), "_x")])
...
### Calculating measure of gesticulation and posture
...
### Wrapping up and Further Readings<a name="furtherreadings"></a>
